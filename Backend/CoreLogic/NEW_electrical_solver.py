# FILE: Backend/CoreLogic/NEW_electrical_solver.py
import numpy as np
import time
import pandas as pd
import os
...
def _history_to_long_dataframe(history: Dict, N_cells: int) -> pd.DataFrame:
    """Convert history dict (lists of arrays) to long DF with cell_id."""
    rows = []
    for t_idx in range(len(history['dt'])):
        for cell_id in range(N_cells):
            row = {
                ...
                'I_cell': history['I_module'][t_idx] / len(history['parallel_groups']),  # Approx equal
                'I_module': history['I_module'][t_idx],
                'V_module': history['V_module'][t_idx],
                'Qgen': history['Qgen_cumulative'][t_idx][cell_id],  # Cumulative per cell
                # DC metadata (repeated)
            }
...
def run_electrical_solver(setup: Dict, dc_table: pd.DataFrame, sim_id: str = None, filename: str = "simulation_results.csv"):
    """Full solver for new DC table (df)."""
    # Unpack
    cells = setup['cells']
    N_cells = len(cells)
    ...
    dc_table = dc_table.copy()  # Avoid modifying input
    n_rows = len(dc_table)
    if n_rows == 0:
        raise ValueError("Empty DC table")

    # Initial states (from cells)
    ...
    history = {
        ...
        'I_module': [],
        'V_module': [],
        'energy_throughput': [],  # cum_energy_kWh per cell
        'Qgen_cumulative': [],  # cum_qgen_Ws per cell
        # DC metadata lists
        'Global Step Index': [],
        'Day_of_year': [],
        'DriveCycle_ID': [],
    ...
    while row_idx < n_rows:
        ...
        current_subcycle = row.get("Subcycle_ID", "")

        # Triggers
        step_triggers = parse_trigger_list_from_row(row, column="step Trigger(s)")
        dc_triggers = parse_trigger_list_from_row(row, column="drive cycle trigger")  # Subcycle/DC level
        ...
        while True:
            ...
            V_parallel_step = np.full(N_cells, np.nan)
            for group_id in parallel_groups:
                ...
                for i, cell_idx in enumerate(group_cells):
                    ...
                    R_eff = (
                        R0
                        + 2.0 * R_p
                        + R1 * (1.0 - np.exp(-dt / tau1))
                        + R2 * (1.0 - np.exp(-dt / tau2))
                    )
                    A[i, i] = R_eff
                    A[i, -1] = 1.0
                    b[i] = K
                ...
                # Current constraint
                A[-1, :N] = 1.0
                b[-1] = I_module_current / n_parallel_groups  # Per group
                ...
                for i, cell_idx in enumerate(group_cells):
                    ...
                    next_SOC = calculate_next_soc(
                        I_cell, dt, capacity_Ah, sim_SOC[cell_idx],
                        coulombic_eff, sim_SOH[cell_idx]
                    )
                    q_irr = (I_cell ** 2) * R0
                    ...
                    V_parallel_step[cell_idx] = V_parallel
            ...
            if num_series > 0:
                v_module = float(np.sum(v_groups) - abs(I_module_current) * R_s * max(0, num_series - 1))
            else:
                v_module = 0.0

            # 4) SAVE HISTORY (before triggers)
            ...
            history["Subcycle_ID"].append(row.get("Subcycle_ID", ""))
            ...
            history["drive cycle trigger"].append(str(row.get("drive cycle trigger", "")))
            history["step Trigger(s)"].append(str(row.get("step Trigger(s)", "")))
            ...
            if per_day_time >= 86400.0:
                per_day_time -= 86400.0  # Reset per day

            # -------------------------------------------------
            # TRIGGER EVALUATION (priority enforced)
            # -------------------------------------------------
            # (A) HARD CUTOFFS: Stop sim after log
            hard_cell_cutoff = (np.any(sim_V_term > HARD_V_cell_max) or np.any(sim_V_term < HARD_V_cell_min))
            hard_pack_cutoff = (v_module > HARD_V_pack_max or v_module < HARD_V_pack_min)
            if hard_cell_cutoff or hard_pack_cutoff:
            ...
            # Action flags with priority: day > dc (subcycle) > step
            ...
            if advance_day:
                # Jump to next day
                j = row_idx + 1
                while j < n_rows and dc_table.iloc[j].get("Day_of_year", None) == current_day:
                    j += 1
                row_idx = j
                break
            if advance_dc:
                # Jump to next DC in day
                j = row_idx + 1
                while j < n_rows:
                    rj = dc_table.iloc[j]
                    if rj.get("Day_of_year", None) != current_day:
                        break
                    if rj.get("DriveCycle_ID", None) != current_dc:
                        break
                    j += 1
                row_idx = j
                break
            ...
            if step_type == "trigger_only":
                pass  # Continue until trigger
            elif step_type == "fixed":
                if step_duration < np.inf and time_in_step >= step_duration:
                    row_idx += 1
                    break
