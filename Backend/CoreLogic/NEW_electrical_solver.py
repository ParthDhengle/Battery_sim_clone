...
def run_electrical_solver(
    ...
    """
    Full solver with:
    - Periodic CSV append every 20 seconds (wall-clock)
    - File-based stop and pause detection (no DB calls)
    - On pause: Flush CSV, create ZIP with CSV + metadata.json, remove signal
    - Proper cleanup on stop/pause/completion
    """
    cells = setup['cells']
    N_cells = len(cells)
    ...
    print(f" Voltage limits: cell [{HARD_V_cell_min:.2f}, {HARD_V_cell_max:.2f}]V, pack [{HARD_V_pack_min:.2f}, {HARD_V_pack_max:.2f}]V")
    ...
    if stop_signal_file and os.path.exists(stop_signal_file):
        os.remove(stop_signal_file)  # Clean old
    if pause_signal_file and os.path.exists(pause_signal_file):
        os.remove(pause_signal_file)  # Clean old

    # Periodic write setup
    last_write_time = time.time()
    last_written_timestep = 0
    WRITE_INTERVAL = 20  # seconds (wall-clock)
    csv_mode = 'w'  # First write creates file, then append

    def write_partial_history(from_idx: int, to_idx: int):
        """Append history[from_idx:to_idx] to CSV file."""
        nonlocal csv_mode, last_written_timestep
        ...
        if csv_mode == 'w':
            csv_mode = 'a'  # Switch to append mode after first write

        last_written_timestep = to_idx
    ...
    while row_idx < n_rows and not sim_terminated and t_global < max_t_global:
        ...
        if pause_signal_file and os.path.exists(pause_signal_file):
            ...
            write_partial_history(last_written_timestep, current_timestep)
            # Create continuation ZIP
            if pack_id and dc_id:
                metadata = {
                    "pack_id": pack_id,
                    "dc_id": dc_id,
                    "last_row": row_idx,
                ...
                try:
                    ...
                    print(f"⏸️ Pause ZIP created: {zip_path}, last_row={row_idx}")
                except Exception as e:
                    print(f"❌ Error creating pause ZIP: {e}")
            else:
                print("⚠️ Warning: Missing pack_id or dc_id; ZIP not created")
            # Remove signal
        ...
        # Periodic CSV write (every 20 seconds wall-clock)
        ...
        inner_iters = 0
        max_inner_iters = int(step_duration / dt_step + 1000) if step_duration < np.inf else 1000000
    ...
    status = 'paused' if 'pause' in locals() and os.path.exists(f"simulations/{sim_id}_pause.zip") else ('stopped by user' if stop_requested else ('terminated early' if sim_terminated else 'completed'))
