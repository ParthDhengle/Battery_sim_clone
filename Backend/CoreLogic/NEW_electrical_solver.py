...
def _history_to_long_dataframe(history: Dict, N_cells: int) -> pd.DataFrame:
    ...
    rows = []
    n_parallel = len(history['parallel_groups'])  # Now safe — we store it!
...
def run_electrical_solver(setup: Dict, dc_table: pd.DataFrame, sim_id: str = None, filename: str = "simulation_results.csv"):
    ...
    # History dict — ADD parallel_groups here!
    history = {
        'dt': [],
        't_global_s': [],
        'SOC': [],
        ...
        'drive cycle trigger': [],
        'step Trigger(s)': [],
        'parallel_groups': parallel_groups,  # ← THIS WAS MISSING!
    }
    ...
    while row_idx < n_rows:
        ...
        while True:
            ...
            for group_id in parallel_groups:
                ...
                A[-1, :N] = 1.0
                b[-1] = I_module_current / n_parallel_groups
            ...
            # HARD CUTOFFS
            if (np.any(sim_V_term > HARD_V_cell_max) or np.any(sim_V_term < HARD_V_cell_min) or
            ...
            if advance_day or advance_dc or advance_step:
                row_idx += 1
                break
