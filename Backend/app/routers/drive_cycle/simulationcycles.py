...
async def generate_simulation_table(sim_id: str):
    ...
    """
    sim = await db.simulations.find_one({"_id": sim_id})
    if not sim:
    ...
    await db.simulations.update_one(
        {"_id": sim_id},
        {"$set": {"simulationTablePath": relative_path}}
    )

    # Optional: return basic stats
    total_days = len(sim.get("calendarAssignments", []))
    total_drive_cycles = len(sim.get("driveCycles", []))
