...
async def load_subcycle_steps_if_needed(sc: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # Robust: ensure .json extension for backward compatibility
    if not file_id.endswith('.json'):
        file_id += '.json'
    abs_path = os.path.join(UPLOAD_SUBCYCLES_DIR, file_id)
    if not os.path.exists(abs_path):
        # Fallback: try without extension or log warning
        alt_path = os.path.join(UPLOAD_SUBCYCLES_DIR, os.path.basename(rel_path))
