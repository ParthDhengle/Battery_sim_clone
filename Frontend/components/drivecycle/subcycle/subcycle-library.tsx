...
export default function SubcycleLibrary({ subcycles, onSubcyclesChange, simId }: SubcycleLibraryProps) {
  ...
  return (
    <div className="space-y-8">
                  ...
                  {subcycle.steps.length > 0 && (
                    <CardContent className="p-0">
                      <div className="flex justify-end p-6 pt-3">
                        <Button size="sm" variant="outline" onClick={() => setViewingTableId(subcycle.id)} disabled={loading}>
                          View Full Table →
                        </Button>
                      </div>
      ...
      {viewingTableId && (
        <FullTable subcycle={subcycles.find(s => s.id === viewingTableId)!} onClose={() => setViewingTableId(null)} />
      )}
    </div>
  )
}
