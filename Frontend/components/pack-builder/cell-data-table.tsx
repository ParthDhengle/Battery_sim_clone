...
export function CellDataTable({
  ...
  return (
    <div className="mt-6 space-y-3 w-full">
      <p className="font-medium text-gray-800">Cell Mapping Details</p>
      {(() => {
        const nRows = Number.parseInt(String(layer.nRows), 10) || 0
        const nCols = Number.parseInt(String(layer.nCols), 10) || 0
        const pitchX = Number.parseFloat(String(layer.pitchX)) || 0
        const pitchY = Number.parseFloat(String(layer.pitchY)) || 0

        if (nRows <= 0 || nCols <= 0 || pitchX <= 0 || pitchY <= 0) {
          return <p className="text-muted-foreground">Invalid layer configuration for table.</p>
        }
        ...
        return (
          <div className="overflow-auto max-h-96 border border-gray-300 rounded-lg shadow-sm">
            <Table>
              ...
              <TableBody>
                {positions.map((pos) => {
                  const rowLeft = isColStaggered
                    ? ""
                    : pos.col > 0
                      ? (rowColToLabel.get(`${pos.row}_${pos.col - 1}`) ?? "")
                      : ""
                  const rowRight = isColStaggered
                    ? ""
                    : pos.col < nCols - 1
                      ? (rowColToLabel.get(`${pos.row}_${pos.col + 1}`) ?? "")
                      : ""
                  const colTop = isRowStaggered
                    ? ""
                    : pos.row > 0
                      ? (rowColToLabel.get(`${pos.row - 1}_${pos.col}`) ?? "")
                      : ""
                  const colBottom = isRowStaggered
                    ? ""
                    : pos.row < nRows - 1
                      ? (rowColToLabel.get(`${pos.row + 1}_${pos.col}`) ?? "")
                      : ""
                  ...
                  if ((pos.row === 0 || pos.row === nRows - 1) && (pos.col === 0 || pos.col === nCols - 1)) {
                    positionType = "Corner"
                  } else if (pos.row === 0 || pos.row === nRows - 1 || pos.col === 0 || pos.col === nCols - 1) {
                    positionType = "Edge"
                  }
                  ...
                  return (
                    <TableRow key={pos.label}>
                      <TableCell className="font-mono">{pos.label}</TableCell>
